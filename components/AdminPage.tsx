...
const AdminPage: React.FC<AdminPageProps> = ({ 
    ...
    return (
        <section id="admin" className="space-y-8">
            ...
            {/* Tabs */}
            <div className="flex justify-center border-b border-blush-pink/10 mb-12 overflow-x-auto no-scrollbar">
                <TabButton 
                    title="Análisis" 
            ...
            <AnimatePresence mode="wait">
                ...
                >
                        ...
                        <div className="space-y-8">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                                <div className="space-y-1">
                                    <h3 className="text-3xl font-serif font-bold text-cocoa-brown">Gestión de Productos</h3>
                                    <p className="text-muted-mauve/60 text-sm">Añade, edita o elimina productos de tu catálogo.</p>
                                </div>
                                <motion.button 
                                    whileHover={{ scale: 1.02 }}
                                    whileTap={{ scale: 0.98 }}
                                    onClick={() => setEditingProduct(emptyProduct)} 
                                    className="bg-cocoa-brown text-white font-bold py-4 px-8 rounded-2xl hover:bg-rose-gold transition-all shadow-xl shadow-cocoa-brown/20 flex items-center gap-3"
                                >
                            ...
                            <motion.div 
                                ...
                                className="bg-gradient-to-br from-rose-gold/10 to-blush-pink/5 border border-rose-gold/20 p-8 rounded-[2.5rem] shadow-sm relative overflow-hidden group"
                            >
                                ...
                                <div className="flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
                                    <div className="w-16 h-16 bg-white rounded-[1.5rem] flex items-center justify-center shadow-lg text-rose-gold shrink-0">
                                        <CheckCircle2 className="w-8 h-8" />
                                    </div>
                                    <div className="space-y-4 text-center md:text-left">
                                        <h4 className="text-2xl font-serif font-bold text-cocoa-brown">¿Cómo crear una Caja Promocional?</h4>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                {products.map(product => (
                                    <motion.div 
                                        key={product.id}
                                        layout
                                        className="bg-white p-6 rounded-[2rem] shadow-xl shadow-cocoa-brown/5 border border-blush-pink/10 flex items-center gap-6 group hover:border-rose-gold/30 transition-all duration-500"
                                    >
                                        <div className="w-20 h-20 rounded-2xl overflow-hidden shrink-0 shadow-md">
                                            <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                        </div>
                                        <div className="flex-grow min-w-0 space-y-1">
                                            <h4 className="font-bold text-cocoa-brown truncate">{product.name}</h4>
                                            <p className="text-xs text-muted-mauve/60 uppercase tracking-widest font-bold">{product.category}</p>
                                            <p className="text-sm font-bold text-rose-gold">${product.price.toLocaleString('es-CL')}</p>
                                        </div>
                                        <div className="flex flex-col gap-2">
                                            <button 
                                                onClick={() => setEditingProduct(product)} 
                                                className="p-3 bg-cream/50 text-cocoa-brown rounded-xl hover:bg-rose-gold hover:text-white transition-all shadow-sm"
                                                title="Editar"
                                            >
                    ...
                    {activeTab === 'options' && currentUserRole === 'superadmin' && (
                        <div className="space-y-8">
                            <div className="space-y-1">
                                <h3 className="text-3xl font-serif font-bold text-cocoa-brown">Personalización</h3>
                                <p className="text-muted-mauve/60 text-sm">Gestiona los sabores, rellenos y colores disponibles para tus productos.</p>
                            </div>
                    ...
                    {activeTab === 'settings' && currentUserRole === 'superadmin' && (
                        <div className="space-y-8">
                            <div className="space-y-1">
                                <h3 className="text-3xl font-serif font-bold text-cocoa-brown">Configuración General</h3>
                                <p className="text-muted-mauve/60 text-sm">Personaliza la identidad visual de tu tienda.</p>
                            </div>
                            
                            <div className="bg-white p-10 rounded-[3rem] shadow-xl shadow-cocoa-brown/5 border border-blush-pink/10">
                                <div className="flex flex-col lg:flex-row gap-12 items-start">
                                    <div className="space-y-6 lg:w-1/3">
                                        <div className="w-20 h-20 bg-rose-gold/10 rounded-[1.5rem] flex items-center justify-center text-rose-gold">
                                            <ImageIcon className="w-10 h-10" />
                                        </div>
                                        <div className="space-y-2">
                                            <h4 className="text-2xl font-serif font-bold text-cocoa-brown">Logotipo del Sitio</h4>
                                            <p className="text-sm text-muted-mauve/60 leading-relaxed">Sube una imagen en formato PNG, JPG o SVG. Se recomienda un fondo transparente para mejor integración.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-grow w-full space-y-8">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                            <div className="space-y-3">
                                                ...
                                                <div className="bg-cream/20 border-2 border-dashed border-blush-pink/20 rounded-[2rem] p-8 flex items-center justify-center h-48">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="bg-white border-2 border-dashed border-rose-gold/20 rounded-[2rem] p-8 flex flex-col items-center justify-center h-48 relative group">
                                                    {newLogoPreview ? (
                                                        <img src={newLogoPreview} alt="Previsualización" className="max-w-full max-h-full object-contain" />
                                                    ) : (
                                                        <div className="text-center space-y-2">
                                                            <ImageIcon className="w-8 h-8 text-rose-gold/30 mx-auto" />
                                                            <p className="text-xs text-muted-mauve/40">Arrastra o selecciona un archivo</p>
                                                        </div>
                                                    )}
                                                    <input 
                                                        id="logoUpload" 
                                        ...
                                        <div className="flex justify-end gap-4">
                                            {newLogoPreview && (
                                                <button 
                                                    ...
                                                    className="px-8 py-4 rounded-2xl text-sm font-bold text-muted-mauve hover:text-red-500 transition-colors"
                                                >
                                                    Cancelar
                                                </button>
                                            )}
                                            <motion.button 
                                                ...
                                                className="bg-cocoa-brown text-white font-bold py-4 px-10 rounded-2xl hover:bg-rose-gold transition-all shadow-xl shadow-cocoa-brown/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-3"
                                            >
                                                {isUploading ? (
            ...
            <AnimatePresence>
                {editingProduct && (
                    ...
                    >
                        ...
                        >
                            {/* Modal Header */}
                            <div className="p-8 border-b border-blush-pink/10 flex justify-between items-center bg-white/50">
                                <div className="space-y-1">
                                    <h3 className="text-3xl font-serif font-bold text-cocoa-brown">
                                        {'id' in editingProduct ? 'Editar' : 'Nuevo'} Producto
                                    </h3>
                                    <p className="text-muted-mauve/60 text-sm">Completa los detalles para tu catálogo.</p>
                                </div>
                                <button 
                                    onClick={() => setEditingProduct(null)}
                                    className="p-3 bg-white rounded-2xl text-muted-mauve hover:text-red-500 transition-all shadow-sm border border-blush-pink/10"
                                >
                                    <X className="w-6 h-6" />
                                </button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-8 space-y-10 custom-scrollbar">
                                ...
                                <div className="space-y-6">
                                    <h4 className="text-xs font-bold text-rose-gold uppercase tracking-[0.2em] ml-1">Información Básica</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select 
                                                ...
                                                className="w-full bg-white border-2 border-blush-pink/20 rounded-2xl py-4 px-6 text-cocoa-brown focus:border-rose-gold focus:ring-4 focus:ring-rose-gold/10 transition-all outline-none appearance-none cursor-pointer"
                                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    <div className="space-y-6">
                                        <h4 className="text-xs font-bold text-rose-gold uppercase tracking-[0.2em] ml-1">Imagen</h4>
                                        <div className="relative group">
                                            <div className="w-full aspect-video rounded-[2rem] overflow-hidden bg-cream/50 border-2 border-dashed border-blush-pink/20 flex items-center justify-center relative">
                                                ...
                                                <div className="absolute inset-0 bg-cocoa-brown/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                                                    <label htmlFor="imageUpload" className="bg-white text-cocoa-brown font-bold py-3 px-6 rounded-2xl cursor-pointer hover:bg-rose-gold hover:text-white transition-all shadow-xl flex items-center gap-2">
                                    ...
                                    <div className="space-y-6">
                                        <h4 className="text-xs font-bold text-rose-gold uppercase tracking-[0.2em] ml-1">Precio Base</h4>
                                ...
                                <div className="space-y-6">
                                    <h4 className="text-xs font-bold text-rose-gold uppercase tracking-[0.2em] ml-1">Personalización Disponible</h4>
                                ...
                                {formState.category === 'promocion' && (
                                    <motion.div 
                                        ...
                                        animate={{ opacity: 1, y: 0 }}
                                        className="p-8 bg-rose-gold/5 border-2 border-dashed border-rose-gold/30 rounded-[2.5rem] space-y-8"
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-rose-gold text-white rounded-2xl flex items-center justify-center shadow-lg">
                                                <Sliders className="w-6 h-6" />
                                            </div>
                                            <h4 className="text-xl font-serif font-bold text-cocoa-brown">Configurador de Caja Promocional</h4>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto p-4 bg-white/50 rounded-3xl border border-blush-pink/10 custom-scrollbar">
                                ...
                                <div className="space-y-6">
                                    <div className="flex justify-between items-center">
                                        <h4 className="text-xs font-bold text-rose-gold uppercase tracking-[0.2em] ml-1">Paquetes de Precios</h4>
                                        <button 
                                            type="button" 
                                            onClick={addTier} 
                                            className="text-xs font-bold text-cocoa-brown bg-white border border-blush-pink/20 py-2 px-4 rounded-xl hover:bg-rose-gold hover:text-white transition-all shadow-sm flex items-center gap-2"
                                        >
                                    ...
                                    <div className="space-y-4">
                                        <AnimatePresence mode="popLayout">
                                            {formState.priceTiers.map((tier, index) => (
                                                <motion.div 
                                                    ...
                                                    className="flex flex-col sm:flex-row items-end gap-4 p-6 bg-white rounded-[2rem] border border-blush-pink/10 shadow-sm group relative"
                                                >
                                                    ...
                                                    <button 
                                                        ...
                                                        className="p-4 text-muted-mauve/30 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all"
                                        ...
                                        {formState.priceTiers.length === 0 && (
                                            <div className="text-center py-10 bg-cream/30 rounded-[2rem] border-2 border-dashed border-blush-pink/10">
                                                <p className="text-sm text-muted-mauve/40 font-medium">No hay paquetes de precios configurados.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {/* Modal Footer */}
                            <div className="p-8 border-t border-blush-pink/10 bg-white/50 flex justify-end gap-4">
                                <button 
                                    type="button" 
                                    onClick={() => setEditingProduct(null)} 
                                    className="px-10 py-4 rounded-2xl text-sm font-bold text-muted-mauve hover:text-cocoa-brown transition-colors"
                                >
                                    Cancelar
                                </button>
                                <motion.button 
                                    ...
                                    className="bg-cocoa-brown text-white font-bold py-4 px-12 rounded-2xl hover:bg-rose-gold transition-all shadow-xl shadow-cocoa-brown/20 flex items-center gap-3"
                                >
                                    <CheckCircle2 className="w-5 h-5" />
                                    Guardar Cambios
                                </motion.button>
...
const AdminCheckbox: React.FC<any> = ({ label, ...props }) => (
    <label className="flex items-center gap-4 p-4 bg-white border border-blush-pink/10 rounded-2xl cursor-pointer hover:bg-cream/50 transition-all group">
...
({title, category, options, newOptionValue, onNewOptionChange, onAdd, onDelete}) => (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-cocoa-brown/5 border border-blush-pink/10 space-y-6">
        <div className="flex items-center justify-between">
            <h4 className="text-xl font-serif font-bold text-cocoa-brown">{title}</h4>
        ...
        <div className="flex gap-3">
            <input 
                ...
                className="flex-grow bg-cream/30 border-2 border-blush-pink/10 rounded-xl py-3 px-4 text-sm focus:border-rose-gold outline-none transition-all"
            />
            <button 
